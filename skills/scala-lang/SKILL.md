---
name: scala-lang
description: Use this skill when working with Scala 3 language features, syntax, type system, metaprogramming, or migrating from Scala 2. Uses braceful syntax exclusively — no significant indentation. All code examples use curly braces with -no-indent compiler option. Covers import aliasing, given/using, structural types, intersection/union types, pattern matching, match types, GADTs, dependent types, inline methods, macros, TASTY inspectors, compile-time operations, and Scala 2-to-3 migration. Trigger when the user mentions Scala 3, Dotty, given/using, inline, macros, match types, GADTs, extension methods, structural types, Scala migration, type-level programming, metaprogramming, TASTY, or any Scala language feature question — even if they don't explicitly say "Scala 3".
---

# Scala 3 Language Features

## Code Style Convention: Braceful Syntax

This skill uses **braceful Scala 3 syntax exclusively** — no significant indentation. Compile with the `-no-indent` flag to enforce this at the compiler level:
...
**Why**: Significant indentation creates fragile diffs, makes refactoring harder, confuses IDEs with copy-paste, and breaks when mixing with braceful libraries. Curly braces are unambiguous and universally understood. See [Make Scala Curly Again](https://makescalacurlyagain.com/) for the full argument.

```scala
// ❌ NO — significant indentation (fragile, harder to refactor)
trait Show[T]:
...
// ✅ YES — braceful syntax (clear, refactor-friendly, universal)
trait Show[T] {
  def show(value: T): String
}
...
**Rules:**
- All definitions use `{ }` — traits, classes, objects, methods, given instances
- All control flow uses `{ }` — if/else, for, match, try/catch
- Use `with` keyword for given instances: `given Show[Int] with { ... }`
- `-no-indent` compiler flag enforces this project-wide
...
Scala 3 supports both braceful and indentation-based syntax. This skill uses braceful syntax exclusively with the `-no-indent` compiler flag. The `using` keyword replaces `implicit` for given instances.

```scala
// Braceful syntax (mandatory with -no-indent)
object A {
  type T = B
  def f(x: B): String = x.toString
}

// if/else, for comprehensions
...
def eval[T](expr: Expr[T]): T = expr match {
  ...
  case IfExpr(c, t, e) => if (eval(c)) { eval(t) } else { eval(e) }
}
```
...
// Scala 3 (braceful — our convention):
object A {
  type T = B
  def f(x: B): String = x.toString
}
...
3. Enforce braceful syntax with `scalacOptions += "-no-indent"` — remove all significant indentation
...
// Enforce braceful syntax project-wide
scalacOptions += "-no-indent"
